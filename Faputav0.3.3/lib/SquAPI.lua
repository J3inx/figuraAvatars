...
function squapi.smoothHeadNeck(element, element2, keeporiginalheadpos)
	...
	function events.render(delta, context)
		...
		element:setOffsetRot(mainheadrot[1]*0.5,mainheadrot[2]*0.5,mainheadrot[3])
    	element2:setOffsetRot(mainheadrot[1]*0.35,mainheadrot[2]*0.3,0)
		if keeporiginalheadpos then 
...
function squapi.eye(element, leftdistance, rightdistance, updistance, downdistance, switchvalues)
	...
	function events.render(delta, context)
		local headrot = (vanilla_model.HEAD:getOriginRot()+180)%360-179
